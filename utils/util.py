...
def recall_func(actual, predicted, topk):
    ...
    for i, v in actual.items():
        ...
        if len(act_set) != 0:
            ...
            true_drugs += 1
    assert num_drugs == true_drugs
    return sum_recall / true_drugs
...
def ndcg_func(ground_truths, ranks):
    ...
    for i, (rank, ground_truth) in enumerate(zip(ranks, ground_truths)):
        ...
        dcg = np.cumsum([1.0 / np.log2(idx + 2) if disease in ground_truth else 0.0 for idx, disease in enumerate(rank)])
        result += dcg / idcg
